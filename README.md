...
- **sae_visualizer.py**
  - Visualizes SAE feature activations for a specific prompt using a heatmap.
  - Uses a threshold-based approach to show only significant feature activations.
  - Implements "Logit Lens" to interpret specific features by projecting them onto the vocabulary and showing the top promoted tokens.

- **feature_sparsity.py**
  - Analyzes the sparsity of SAE features across a large dataset (e.g., Wikitext-2).
  - Calculates the activation frequency of each feature.
  - Tracks the specific tokens that trigger each feature.
  - Saves the analysis results (frequencies, counts, top tokens) to `feature_sparsity.csv` and `feature_sparsity_data.pt`.
  - Generates a histogram of feature activation frequencies (`sparsity_histogram.png`).

- **feature_analysis.ipynb**
  - A Jupyter Notebook for further analysis and visualization of the data generated by `feature_sparsity.py`.
  - Loads `feature_sparsity_data.pt` to perform custom plotting, such as Power Law fits of feature sparsity.

- **logit_lens.py**
  - Applies the Logit Lens technique to the model's intermediate layers.
  - Visualizes the model's "thought process" by showing the predicted token at each layer for a given input prompt.

- **test_generation.py**
  - Demonstrates autoregressive text generation using the model.
  - Generates a continuation of a given prompt to verify the model's generation capabilities.
